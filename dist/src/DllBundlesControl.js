...
var DllBundlesControl = (function () {
    ...
    DllBundlesControl.prototype.checkBundles = function () {
        ...
        return this.analyzeModulesState()
            ...
            var state = _this.getBundlesState();
            // make a list of all bundles that are invalid, this is not related to the analyzed data
            var invalidBundleNames = _this.bundles
                .map(function (b) { return b.name; })
                .filter(function (name) {
                return !_this.isValidBundleCacheId(name, state) || !_this.bundleLooksValid(name);
            });
            ...
            analyzed.added.concat(analyzed.changed).concat(analyzed.removed).concat(analyzed.error)
                .forEach(function (p) { return invalidBundleNames.indexOf(p.bundle) === -1 && invalidBundleNames.push(p.bundle); });
    ...
    DllBundlesControl.prototype.saveBundleState = function () {
        ...
        return this.getMetadata()
            ...
            var bundlesState = {};
            bundlesState.cacheIds = _this.bundles.reduce(function (ids, bundle) {
                ids[bundle.name] = _this.getBundleCacheId(bundle.name);
                return ids;
            }, {});
            ...
            fs.writeFileSync(Path.join(_this.options.dllDir, BUNDLE_STATE_FILENAME), JSON.stringify(bundlesState, null, 2));
        });
    };
    /**
    ...
    DllBundlesControl.prototype.isValidBundleCacheId = function (name, state) {
        var stateBundleCacheId = state && state.cacheIds && state.cacheIds[name];
        return this.getBundleCacheId(name) === stateBundleCacheId;
    };
    DllBundlesControl.prototype.getBundleCacheId = function (bundleName) {
        var bundleCacheId = this.options.bundleCacheId;
     ...
     */
    DllBundlesControl.prototype.analyzeModulesState = function () {
        var _this = this;
        return this.getMetadata() // get metadata for the required bundle configuration
            ...
            var bundlesStateModules = _this.getBundlesState().modules;
            var pkgCache = {
            ...
            packages.forEach(function (pkgInfo) {
                ...
                else if (bundlesStateModules.hasOwnProperty(pkgInfo.name)) {
             ...
             */
            result.removed = Object.keys(bundlesStateModules).map(function (k) { return new PackageInfo(bundlesStateModules[k].bundle, k); });
            return result;
        });
    };
    /**
     ...
     */
    DllBundlesControl.prototype.getBundlesState = function () {
        if (fs.existsSync(Path.join(this.options.dllDir, BUNDLE_STATE_FILENAME))) {
            var state = jsonfile.readFileSync(Path.join(this.options.dllDir, BUNDLE_STATE_FILENAME));
            if (!state.cacheIds) {
                state.cacheIds = {};
            }
            if (!state.modules) {
